...
def main(args):
    ...
    rel_dt = {k: [] for k in args.motif_dt}
    ...
    for line in fasta:
        if line.startswith('>') and seq:
            ...
            for mot, pos_ls in seq_dt_chrom.items():
                rel_dt[mot] = rel_dt[mot] + [(chr_name, pos-begin) for pos in pos_ls]

            begin += len(seq)
            seq = ''
            chr_name = line.rstrip().split(' ')[0][1:]
    ...
    m1 = args.m1[0].replace('/', '')
    m2 = args.m2[0].replace('/', '')
    m1_df = pd.DataFrame(rel_dt[m1], columns = ['chr', 'pos'])
    m1_df['enzyme'] = 1
    m2_df = pd.DataFrame(rel_dt[m2], columns = ['chr', 'pos'])
    m2_df['enzyme'] = 2
    df = pd.concat([m1_df, m2_df])
    df.to_csv(os.path.join(args.o, 'cut_sites_' + os.path.basename(args.genome) + '.csv'), index=None)
...
def check_for_enzymes(args):
    with open(os.path.join(os.path.dirname(__file__),
              'type_iip_enzymes.pickle'), 'rb') as type_ii_file:
