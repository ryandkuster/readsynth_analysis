...
def main():
    ...
    for name, binom in tax_df.groupby(['genus', 'species']):
        ...
        if unclassified:
            ...
            tmp_df = check_refseq_category_s(df, g, s)
            if tmp_df.shape[0] < targets:
                tmp_df = check_assembly_level_s(df, g, s)
    ...
    for g in u_df['genus'].unique():
        ...
        tmp_df = check_refseq_category_g(df, g)
        if tmp_df.shape[0] < targets:
            tmp_df = check_assembly_level_g(df, g)
...
def check_refseq_category_s(df, g, s):
    refseq_ls = ['representative genome', 'reference genome']
    for i in refseq_ls:
        tmp_df = df.loc[(df['organism_name'].str.contains(s)) & (df['organism_name'].str.contains(g)) & (df['refseq_category'] == i)]
        if tmp_df.shape[0] >= 1:
            break
    return tmp_df 


def check_assembly_level_s(df, g, s):
    assembly_ls = ['Complete Genome', 'Chromosome', 'Scaffold', 'Contig']
    for i in assembly_ls:
        tmp_df = df.loc[(df['organism_name'].str.contains(s)) & (df['organism_name'].str.contains(g)) & (df['assembly_level'] == i)]
        if tmp_df.shape[0] >= 1:
            break
    return tmp_df 


def check_refseq_category_g(df, g):
    refseq_ls = ['representative genome', 'reference genome']
    for i in refseq_ls:
        #tmp_df = df.loc[(df['organism_name'].str.contains(g)) & (df['refseq_category'] == i)]
        tmp_df = df.loc[(df['organism_name'].str.startswith(g)) & (df['refseq_category'] == i)]
        if tmp_df.shape[0] >= 1:
            break
    return tmp_df 


def check_assembly_level_g(df, g):
    assembly_ls = ['Complete Genome', 'Chromosome', 'Scaffold', 'Contig']
    for i in assembly_ls:
        #tmp_df = df.loc[(df['organism_name'].str.contains(g)) & (df['assembly_level'] == i)]
        tmp_df = df.loc[(df['organism_name'].str.startswith(g)) & (df['assembly_level'] == i)]
        if tmp_df.shape[0] >= 1:
            break
    return tmp_df 
